...
def PR_curve(truth, scores):
    # a custom function to manually vary threshold for PR curve
    # fixes the weird drop at the end

    sorted_scores = np.sort(scores)

    precision, recall = [], []

    #vary thresholds and calculate TP, FP
    for s in sorted_scores:
        TP = truth[scores >= s].sum()
        FP = len(truth[scores >= s]) - TP
        FN = truth[scores < s].sum()
